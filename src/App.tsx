...
export function App() {
  ...
  useEffect(() => {
    const carregarDadosIniciais = async () => {
      if (isAuthenticated && user && !dadosCarregados) {
        console.log('🔄 Carregando dados do Supabase...');
        const dados = await carregarTudo();
        
        if (dados) {
          console.log('✅ Dados recebidos:', dados);
          if (dados.atribuicoes?.length) importData(dados.atribuicoes);
          if (dados.escola) restaurarEscola(dados.escola);
          if (dados.docentes?.length) restaurarDocentes(dados.docentes);
          if (dados.alunos?.length) restaurarAlunos(dados.alunos);
          if (dados.servidores?.length) restaurarServidores(dados.servidores);
  ...
  }, [isAuthenticated, user, dadosCarregados, carregarTudo, importData, restaurarEscola, restaurarDocentes, restaurarAlunos, restaurarServidores]);

  // Marcar que tem alterações quando os dados mudarem
  useEffect(() => {
    if (dadosCarregados) {
      setTemAlteracoes(true);
    }
  }, [atribuicoes, escola, docentes, alunos, servidores, dadosCarregados]);
  ...
  const handleSaveAll = async () => {
    const dadosParaSalvar = {
      ...
      bloqueios: [],
      horarios: [],
      projetos: escola.projetos || []
    };
    
    console.log('💾 Salvando dados:', dadosParaSalvar);
    const sucesso = await salvarTudo(dadosParaSalvar);
    
    if (sucesso) {
      alert('✅ Dados salvos com sucesso!');
      setTemAlteracoes(false);
    } else {
      alert('❌ Erro ao salvar: ' + (erroDatabase || 'Erro desconhecido'));
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          {ultimoSave && (
            <span className="text-xs text-gray-500 hidden sm:block">
              Último save: {ultimoSave.toLocaleTimeString()}
            </span>
          )}
          {temAlteracoes && (
